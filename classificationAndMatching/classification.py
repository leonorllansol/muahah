import pickle, os, re, copy, csv, treetaggerwrapper, inspect
...
def model_training(pandas_query, pandas_label, classifier, vectorizer, corpus_name: str, query_or_answer: str):
    ...
    file_path = parent_dir + 'models/' + query_or_answer + '/' + corpus_name + "_model.sav"
...
def model_loader(corpus_name: str, query_or_answer: str):
    ...
    path = parent_dir + '/models/' + query_or_answer + '/' +  corpus_name + "_model.sav"
...
def train(corpora_dict: dict, query_or_answer: str,
    ...
    for corpus in corpora_dict[query_or_answer]:
        ...
        if corpora_dict[query_or_answer][corpus]['corpusType'] == 'specific':
            ...
            generic_pandas = pd.read_csv(parent_dir + generic_corpus_path, encoding='utf-8', sep='*')
...
def predict(corpora_dict: dict, query: str, query_or_answer: str) -> list:
    ...
    for corpus in corpora_dict:
        ...
        if 'sublabel' in corpora_dict[corpus]:
            file_name = parent_dir + 'models/' + query_or_answer + '/' + prediction + '_model.sav'
            ...
            while os.path.isfile(file_name):
                ...
                list_of_predictions.append(prediction)
                file_name = parent_dir + 'models/' + query_or_answer + '/' + prediction
...
def rules(query: str, previous_prediction: str) -> None or str:
    ...
    wh_questions_file = open(parent_dir + 'corpora/WH_QUESTIONS.txt', 'r')
    list_questions_file = open(parent_dir + 'corpora/LIST_QUESTIONS.txt', 'r')
    wh_list = [s.rstrip() for s in list(wh_questions_file)]
    list_list = [s.rstrip() for s in list(list_questions_file)]

    tagger = treetaggerwrapper.TreeTagger(TAGPARFILE=parent_dir + 'resources/TREETAGGER/lib/portuguese.par', TAGDIR=parent_dir + 'resources/TREETAGGER/')
