...
class AntiCovidAgent:
    def __init__(self,configs):
        ...
        # {query: {0: ..., 1: ...}, source:{0: ..., 1: ...}}
        querySources = pd.read_csv(four_up + "/corpora/covid_source.csv", sep="*").to_dict()
        # {query: {word1: ..., word2: ...}}
        self.tfidf_scores = tfidf.getTfIdfScore(self.stopwordsPath,list(querySources['query'].values()))

        self.querySources = {}
        for i in querySources['query']:
            query = querySources['query'][i]
            source = querySources['source'][i]
            self.querySources[query] = source
    ...
    def requestAnswer(self,query,candidates, query_labels, covid_labels):
        found_covid_label = False
        query_covid_label = ""

        print(query_labels)
        for label in query_labels:
            if label in covid_labels:
                found_covid_label = True
                query_covid_label = label
                self.context.append(label)

        # if not found_covid_label:
        ...
        if(len(candidates) > 0):
            ...
            for c in candidates:
                ...
                # separator = " "
                # sim_score = edit_distance(separator.join(q),separator.join(d))
            ...
            if len(self.context) > 1:
                ...
                for c in top_candidates:
                    ...
                    #if query_covid_label == answer_covid_label:
                ...
                if len(top_candidates_same_label) == 0:
                    return top_candidates[0].getAnswer(), self.querySources[top_candidates[0].getQuestion()], top_candidates[0].getQuestion()
                else:
                    return top_candidates_same_label[0].getAnswer(), self.querySources[top_candidates_same_label[0].getQuestion()], top_candidates_same_label[0].getQuestion()
        ...
        return bestPair.getAnswer(), self.querySources[bestPair.getQuestion()], bestPair.getQuestion()
